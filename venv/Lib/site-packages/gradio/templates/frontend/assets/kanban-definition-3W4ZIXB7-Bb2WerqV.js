...
`+e);const E=s.db.getData(),m=H();m.htmlLabels=!1;const l=fe(u),D=l.append("g");D.attr("class","sections");const I=l.append("g");I.attr("class","items");const g=E.nodes.filter(b=>b.isGroup);let w=0;const _=10,F=[];let N=25;for(const b of g){const A=m?.kanban?.sectionWidth||200;w=w+1,b.x=A*w+(w-1)*_/2,b.width=A,b.y=0,b.height=A*3,b.rx=5,b.ry=5,b.cssClasses=b.cssClasses+" section-"+w;const L=await ye(D,b);N=Math.max(N,L?.labelBBox?.height),F.push(L)}let V=0;for(const b of g){const A=F[V];V=V+1;const L=m?.kanban?.sectionWidth||200,M=-L*3/2+N;let T=M;const Y=E.nodes.filter(i=>i.parentId===b.id);for(const i of Y){if(i.isGroup)throw new Error("Groups within groups are not allowed in Kanban diagrams");i.x=b.x,i.width=L-1.5*_;const r=(await me(I,i,{config:m})).node().getBBox();i.y=T+r.height/2,await be(i),T=i.y+r.height/2+_/2}const B=A.cluster.select("rect"),O=Math.max(T-M+3*_,50)+(N-25);B.attr("height",O)}_e(void 0,l,m.mindmap?.padding??K.kanban.padding,m.mindmap?.useMaxWidth??K.kanban.useMaxWidth)},"draw"),Ge={draw:Be},je=o(e=>{let u="";for(let s=0;s<e.THEME_COLOR_LIMIT;s++)e["lineColor"+s]=e["lineColor"+s]||e["cScaleInv"+s],Se(e["lineColor"+s])?e["lineColor"+s]=ce(e["lineColor"+s],20):e["lineColor"+s]=le(e["lineColor"+s],20);const p=o((s,d)=>e.darkMode?le(s,d):ce(s,d),"adjuster");for(let s=0;s<e.THEME_COLOR_LIMIT;s++){const d=""+(17-3*s);u+=`
  ...
  .kanban-label {
    ...
    ${Ne()}
`,"getStyles"),Me=Fe,gt={db:Ve,renderer:Ge,parser:xe,styles:Me};export{gt as diagram};
